      ...
      {products && products.length>0 && (
        <div style={{marginTop:26}}>
          <div style={{textAlign:'center', marginBottom:14}}>
            <div style={{fontSize:11, fontWeight:700, letterSpacing:'0.14em', textTransform:'uppercase', color:'#6B6A78'}}>{t('pricing.products.kicker')}</div>
            <div style={{marginTop:6, fontSize:13, color:'#9A99A6'}}>{t('pricing.products.subtitle')}</div>
          </div>
              ...
              <div key={prod.key} className="m-card" style={{padding:18}}>
                <div style={{display:'flex', gap:12, alignItems:'center'}}>
                  ...
                  <div style={{minWidth:0}}>
                    <div style={{fontSize:15, fontWeight:750, color:'#F5F5F7'}}>{lang==='ar'? prod.nameAr : prod.name}</div>
                    <div style={{fontSize:12.5, color:'#9A99A6', lineHeight:1.4, marginTop:2}}>{lang==='ar'? prod.taglineAr : prod.tagline}</div>
                  </div>
                </div>
                <div style={{marginTop:12, display:'flex', alignItems:'baseline', gap:6}}>
                  <span style={{fontSize:22, fontWeight:850, color:'#F5F5F7'}}>${prod.price}</span>
                  <span style={{fontSize:11, color:'#6B6A78'}}>{t('pricing.oneTime')}</span>
                </div>
                ...
                <div style={{marginTop:14}}>
                  {prod.comingSoon? <div style={{textAlign:'center', padding:'11px', borderRadius:12, background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.06)', color:'#6B6A78', fontWeight:600}}>{t('pricing.comingSoon')}</div>
      ...
      {hostingModal && (
        <div onClick={()=>setHostingModal(null)} style={{position:'fixed', inset:0, zIndex:80, background:'rgba(0,0,0,0.62)', backdropFilter:'blur(10px)', display:'flex', alignItems:'flex-end', padding:16}}>
          <div onClick={e=>e.stopPropagation()} className="m-card" style={{width:'100%', padding:20, borderRadius:20, maxWidth:520, margin:'0 auto'}}>
            ...
            <div style={{textAlign:'center', fontWeight:800, color:'#F5F5F7'}}>{lang==='ar'? hostingModal.nameAr : hostingModal.name}</div>
            <div style={{textAlign:'center', fontSize:12.5, color:'#9A99A6', marginTop:4}}>{t('pricing.hosting.chooseType')}</div>
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginTop:16}}>
              <button type="button" onClick={()=>{ setHostingModal(null); if(!user){ navigate('/login', {state:{from:`/checkout?plan=${hostingModal.key}&cycle=monthly`}}); return;} navigate(`/checkout?plan=${hostingModal.key}&cycle=monthly`); }} style={{padding:16, borderRadius:14, border:'1px solid rgba(16,185,129,0.18)', background:'rgba(16,185,129,0.06)', textAlign:'center'}}>
                <div style={{fontSize:22}}>🏠</div><div style={{marginTop:6, fontWeight:700, color:'#6EE7B7', fontSize:13}}>{t('pricing.hosting.localShort')}</div><div style={{fontSize:11, color:'#9A99A6', marginTop:2}}>{t('pricing.hosting.free')}</div><div style={{marginTop:8, fontSize:13, fontWeight:800, color:'#10B981'}}>$0</div>
              </button>
              <button type="button" onClick={()=>{ setHostingModal(null); if(!user){ navigate('/login', {state:{from:`/checkout?plan=${hostingModal.key}&cycle=monthly&cloud=1`}}); return;} navigate(`/checkout?plan=${hostingModal.key}&cycle=monthly&cloud=1`); }} style={{padding:16, borderRadius:14, border:'1px solid rgba(124,58,237,0.22)', background:'rgba(124,58,237,0.08)', textAlign:'center'}}>
                <div style={{fontSize:22}}>☁️</div><div style={{marginTop:6, fontWeight:700, color:'#A78BFA', fontSize:13}}>{t('pricing.hosting.cloudShort')}</div><div style={{fontSize:11, color:'#9A99A6', marginTop:2}}>24/7</div><div style={{marginTop:8, fontSize:13, fontWeight:800, color:'#A78BFA'}}>$8<span style={{fontSize:11, fontWeight:500, color:'#9A99A6'}}>{t('pricing.perMonthShort')}</span></div>
              </button>
            </div>
            <button type="button" onClick={()=>setHostingModal(null)} style={{width:'100%', marginTop:12, padding:12, borderRadius:12, border:'1px solid rgba(255,255,255,0.07)', background:'rgba(255,255,255,0.04)', color:'#9A99A6', fontWeight:600}}>{t('pricing.cancel')}</button>
          </div>
        </div>
      )}
